...
class RedisEnhancedHttpServer {
  constructor() {
    this.server = new Server(
      {
        name: "redis-enhanced-http-server",
        version: "1.0.0",
      },
      {
        capabilities: {
  ...
  setupExpress() {
    ...
    this.app.get('/health', (req, res) => {
      res.json({ status: 'ok', service: 'redis-enhanced-mcp-server' });
    });
  }
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      try {
        switch (name) {
          ...
          // TYPE DETECTION
          case "redis_type":
            const typeResult = await this.client.type(args.key);
            return { content: [{ type: "text", text: typeResult }] };
  ...
  async run() {
    ...
    const transport = new SSEServerTransport("/message", this.app);
    await this.server.connect(transport);
    console.log("Redis Enhanced MCP server running with HTTP/SSE transport");
  }
}

const server = new RedisEnhancedHttpServer();
